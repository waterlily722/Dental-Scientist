...
def _collect_runs(base_dir: Path):
    ...
    for run_dir in sorted(base_dir.glob("run_*")):
        ...
        if loaded is None:
            continue
        rows.append((run_dir.name, *loaded))
    return rows
...
def _plot_primary_curves(runs, out_dir: Path):
    ...
    for run_name, dataset_name, _payload, all_results in runs:
        ...
        plt.plot([x["epoch"] for x in val_log], [x.get("primary_metric", float("nan")) for x in val_log], label=f"{run_name} val mAP")
    plt.xlabel("Epoch")
    plt.ylabel("Value")
    plt.title("Dental Detection Training Loss and Validation mAP")
    plt.legend()
    plt.tight_layout()
...
def _plot_metric_curves(runs, out_dir: Path):
    ...
    fig.suptitle("Dental Detection Validation Metrics")
...
def _plot_summary(runs, out_dir: Path):
    ...
    plt.ylim(0.0, 1.0)
    plt.ylabel("Score")
    plt.title("Dental Detection Test Metrics by Run")
    plt.legend()
    plt.tight_layout()
