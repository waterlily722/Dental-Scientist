...
METRICS = ["mean_distance", "pck@0.05"]
...
def _plot_primary_curves(runs, out_dir: Path):
    ...
    for run_name, dataset_name, _payload, all_results in runs:
        ...
        plt.plot([x["epoch"] for x in val_log], [x.get("primary_metric", float("nan")) for x in val_log], label=f"{run_name} val score")
    plt.xlabel("Epoch")
    plt.ylabel("Value")
    plt.title("Dental Keypoint Training Loss and Validation Score")
    plt.legend()
    plt.tight_layout()
...
def _plot_metric_curves(runs, out_dir: Path):
    ...
    fig.suptitle("Dental Keypoint Validation Metrics")
...
def _plot_summary(runs, out_dir: Path):
    ...
    width = 0.35
    mean_distance = [payload.get("best_test_reported_metrics", {}).get("mean_distance", float("nan")) for _, _, payload, _ in runs]
    pck = [payload.get("best_test_reported_metrics", {}).get("pck@0.05", float("nan")) for _, _, payload, _ in runs]

    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    axes[0].bar(x, mean_distance, width=width)
    axes[0].set_title("Mean Distance")
    axes[0].set_xticks(x)
    axes[0].set_xticklabels(names, rotation=25, ha="right")

    axes[1].bar(x, pck, width=width)
    axes[1].set_title("PCK@0.05")
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(names, rotation=25, ha="right")
    axes[1].set_ylim(0.0, 1.0)

    fig.suptitle("Dental Keypoint Test Metrics by Run")
    fig.tight_layout(rect=(0, 0, 1, 0.92))
    fig.savefig(out_dir / "summary_metrics.png", dpi=200)
    plt.close(fig)
